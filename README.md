...
- 两种色彩模式：默认模式与色卡标定配置。标定配置明确要求 `relative-density-log10 → linear-srgb-d65`，可携带三条反特性曲线、3×3 矩阵和可选 3D LUT。
...
- 色卡自动工作流：针对已旋正、透视校正的 6×4 ColorChecker Classic 照片，自动定位规则网格、在色块中央做 MAD 异常值剔除采样，并拟合/保存单调密度特性曲线与 3×3 色彩矩阵；色块位置、采样和拟合都可在核心单独测试。针对特定胶卷仍应在受控拍摄条件下复核，并按需要导入完整特性曲线/LUT。
...
- 默认模式高密度锚点：显式 Dmax ROI 会额外保存中性区域的 R/G/B 密度范围；标准模式默认不从主体高光猜测逐通道 Dmax，只有在高级设置显式打开“自动中和 Dmax”后才启用低色度尾部启发式。冻结结果可随整卷配方复用，并由 CPU/GPU 预览、母版及回退路径共同执行。默认预饱和 1.08 也会随项目配方贯通两条渲染路径。
...
- 输出色彩可信度：默认模式始终标为“未校准”；色卡校准模式只有在来源相机、解码器、镜头、胶片、冲洗工艺和光源等完整拍摄上下文都匹配配置时才是“设备匹配”，其余情况标为“配置未验证”。sRGB ICC 只声明交付编码，不证明上游相机颜色准确；DNG 因此仅允许设备匹配输出。
...
Photon Transfer Curve 描述读出噪声、光子散粒噪声、转换增益和响应非均匀性，不描述传感器的光谱响应。因此 A7R V PTC 优化只提高低信噪比颜色的稳定性，不能让默认模式升级为“颜色准确”；相机到线性 sRGB 的色彩含义仍必须来自设备匹配的色卡配置。Photons to Photos 也提示其实测读噪可能受黑电平裁切影响，所以该模型只在完全匹配的机型和 ISO 上启用，并保留明确提示。
...
默认模式输出为 `uncalibrated`；配置缺失或相机/解码器不匹配时为 `profile-unverified`；只有相机型号与 `decoderFingerprint` 同时匹配色卡标定配置时才是 `device-matched`。TIFF、JPEG、HEIF 中嵌入的 sRGB ICC 只说明交付编码，不会把未表征的相机 RGB 变成颜色准确的 sRGB。
...
→ 最终裁切与相对密度
→ 默认模式片基 + 中性高密度锚点反转 / 色卡标定配置反转（标定曲线保持绝对密度域；
  曲线端点按对数域斜率外推；串扰矩阵负值在此钳零；白平衡先于 3D LUT）
→ 场景线性正像
→ 亮度域曝光、绕 0.18 中灰的对数域对比度、高光压缩、饱和度（超出显示上限的通道始终等比压缩保色相）
