...
import type { ExportResult } from "../shared/ipc.ts";
import { decodeSource } from "./decode.ts";
import { writeTiff16 } from "./tiff-write.ts";

/**
 * Re-decodes the source at full resolution, runs the shared core pipeline
 * with the renderer's recipe, quantizes to sRGB and writes the container.
 * TIFF delivers 16-bit sRGB via the internal encoder; JPEG is 8-bit sRGB
 * at quality 95 with 4:4:4 chroma. Neither carries an embedded profile;
